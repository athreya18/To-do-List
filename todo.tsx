...
const Todos = (props: any) => {
    ...
    const update = () => {
        console.log({title, desc, selectedTaskIndex})
        editTodoTasks(title, desc, false, selectedTaskIndex);
        setSelectedTaskIndex(0);
        closeDialog();
    };
                            ...
                            <div key={index} className='mt-5 bg-[rgba(245,247,249,1)] bg-opacity-100 flex flex-col justify-center items-center w-3/4 h-28 rounded-2xl relative'>
                                {task.showEdit && (
                                    <>
                                        ...
                                        <Image src={deleteimg} alt="" onClick={() => { deleteTask(task.title) }} width={28} height={28} className="absolute top-10 right-3 cursor-pointer"></Image>
                                        {/* Edit */}
                                    </>
                                )}
                                <div className='absolute flex flex-row justify-start self-start left-5 top-8'>
                                    <div className="">
                                        <Checkbox id={`checkbox-${index}`} checked={task.showEdit} onCheckedChange={() => {
                                            editTodoTasks(task.title, task.description, !task.showEdit, index)
